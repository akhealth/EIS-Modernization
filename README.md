...
* There is a risk that we won't be able to deploy to production frequently enough that we can work in an agile fashion. We will de-risk this via prototyping and through [developing a DevOps strategy](https://github.com/AlaskaDHSS/DevSecOpsMvp#devsecops-mvp) with the State of Alaska.
...
* There is a risk that the ATO process will prevent us from being able to deploy continuously to a production environment. We plan to mitigate this risk by understanding the path to ATO and compensating with negotiating release cycles on the existing production environment that are as small as possible until we are able to authorize the new environment. See [DevSecOps repo](https://github.com/AlaskaDHSS/DevSecOpsMvp#devsecops-mvp) for more information.
* There is a risk that by having multiple vendors contributing to the same codebase, we increase the complexity of integration. There could be problems that arise specifically from the logisitics and timing of multiple contributors. We plan to de-risk this by [working transparently](https://github.com/AlaskaDHSS/acq-alaska-dhss-modernization/blob/master/how-we-work.md), and using [shared tools and processes](https://github.com/AlaskaDHSS/acq-alaska-dhss-modernization/blob/master/DevOps.md) that assist with preventing conflicts and automating testing.
